...
for(t in 1:T){  
	...
	while(i <= N){ # While the number of accepted particles is less than N_particles
      	...
      	if(prior.non.zero(c(N0_star,age_sh_star,age_rt_star,beta_star,f_E_star))) {
    			...
    			for(j in 1:n){
    				...
    				distance[j,] <-dist    
    				if((dist[1] <= epsilon_T[t]) & (dist[2] <= epsilon_A[t])){ # If both distances are less than their tolerances
    					m<-m+1
    				}
    			}	
    			if (m>0){
	...
	w.old<-w.new/sum(w.new)
 	...
 	write.csv(res.new, file = paste("results_case_2_ABC_SMC_MNN_pop_",t,".csv",sep=""), row.names=FALSE)
}
